...
for my $cousin ( keys %patocousins){
	...
	$patocousinsa{$cousin}=wavg(@values);#weighted average 
...
#take 2 words from arguments
  ...
  #check the 2 words are not empty string
  ...
  #to see if word1 and word2 have adjective senses
  @w1options = &getWNSynsets($word1, "a");
  @w2options = &getWNSynsets($word2, "a");
  
  #if word1 does not have adjective senses
  if(!scalar(@w1options)){
	@w1options = &getWNSynsets($word1, "nvar");
  }

  #if word2 does not have adjective senses
  if(!scalar(@w2options)){
	 @w2options = &getWNSynsets($word2, "nvar");
  }
  
  #if word1 or word2 cannot be found in wordnet
  ...
  #see if current measure can handle the POS
...
sub printusage{
	...
	\t3. The measure method you want to use. (could be lesk or vector)\n
